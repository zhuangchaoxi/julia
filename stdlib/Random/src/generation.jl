...
function SamplerBigInt(::Type{RNG}, r::AbstractUnitRange{BigInt}, N::Repetition=Val(Inf)
    ...
    highbits > 0 && (nlimbs += 1)
    mhigh = nlimbs == 0 ? zero(Limb) : GC.@preserve m unsafe_load(m.d, nlimbs)
    highsp = Sampler(RNG, Limb(0):mhigh, N)
...
function rand!(rng::AbstractRNG, x::BigInt, sp::SamplerBigInt)
    ...
    GC.@preserve x begin
        ...
        hm = unsafe_load(sp.m.d, nlimbs)
        while true
            rand!(rng, limbs)
            hx = limbs[nlimbs] = rand(rng, sp.highsp)
            hx < hm && break # avoid calling mpn_cmp in the general case
