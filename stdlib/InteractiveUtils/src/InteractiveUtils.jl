...
"""
function varinfo(m::Module=Base.get_active_module(), pattern::Regex=r"")
    rows =
        Any[ let value = getfield(m, v)
                 Any[string(v),
                     (value===Base || value===get_active_module() || value===Core ? "" : format_bytes(summarysize(value))),
                     summary(value)]
             end
...
function dumptype(io::IO, @nospecialize(x), n::Int, indent)
    ...
    isa(x, DataType) && x.abstract && dumpsubtypes(io, x, Base.get_active_module(), n, indent)
    nothing
end
